    ...
    $data .= "
      <tr class='align-middle'>
        ...
        <td>
          ...
          <button type='button' onclick='remove_room({$row['id']})' class='btn btn-danger shadow-none btn-sm' room-images'>
...
if (isset($_POST['thumb_image'])) {
  ...
  // Set all existing images for this room to thumb = 0
  $pre_q = "UPDATE `room_images` SET `thumb` = ? WHERE `room_id` = ?";
  $pre_v = [0, $frm_data['room_id']];
  $pre_res = update($pre_q, $pre_v, 'ii');

  // Set the selected image as the new thumbnail (thumb = 1)
