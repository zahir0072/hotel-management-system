...
if (isset($_POST['get_bookings'])) {
  ...
  $query = "SELECT bo.*, bd.* FROM `booking_order` bo
    ...
    WHERE (bd.user_name LIKE ? OR bd.phonenum LIKE ?)
    AND (bo.booking_status=? AND bo.arrival=?) 
    ORDER BY bo.booking_id ASC";

  $res = select($query, ["%$frm_data[search]%", "%$frm_data[search]%", "booking", 0], 'sssi');
...
if (isset($_POST['assign_room'])) {
  ...
  $res = update($query, $values, 'isi'); //it will update 2 rowsso it will return 2
